...
def get_user_activity_metrics():
    statsig_user_activity_daily = get_asset_key_for_model(
        [get_dbt_partitioned_models()], "statsig_user_activity_daily"
    )
    ...
    def user_activity_metrics(
        ...
        with snowflake.get_connection() as conn:
            ...
            for batch in cur.fetch_pandas_batches():
                ...
                context.log.info(
                    f"Statsig write events response: {result.status_code} {result.text}"
                )
...
@cache
def get_org_performance_metrics():
    cloud_product_organizations = get_asset_key_for_model(
        [get_dbt_non_partitioned_models()], "cloud_product_organizations"
    )
    ...
    return org_performance_metrics
