...
from dagster_fivetran import FivetranConnectorTableProps, FivetranResource
...
def load(context: ComponentLoadContext) -> FivetranComponent:
    def _translate(
        ...
        return base_asset_spec.replace_attributes(
            ...
            metadata={"group_name": f"fivetran_{'_'.join(props.table.split('.')[:-1])}"}
        )

    return FivetranComponent(
        instance=FivetranResource(
