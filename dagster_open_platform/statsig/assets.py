...
def user_activity_daily(
    ...
    _, end_timestamp = (
        int(datetime.strptime(context.partition_key_range.start, "%Y-%m-%d").timestamp()),
        int(datetime.strptime(context.partition_key_range.end, "%Y-%m-%d").timestamp()),
    )
    ...
    with snowflake.get_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            f"SELECT * FROM {qualified_name} WHERE DS = '{context.partition_key_range.start}'"
        )
        for batch in cur.fetch_pandas_batches():
            statsig.write_events(
                [
